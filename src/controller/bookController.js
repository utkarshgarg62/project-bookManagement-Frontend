...
const createBook = async function (req, res) {
  ...
  try {
    ...
    if (!isValid(subcategory)) {
      return res
        .status(400)
        .send({ status: false, message: "Enter subcategory" });
    }
...
const getBooksDataById = async function (req, res) {
  ...
  try {
    ...
    if (Object.keys(BookId).length < 1)
      return res
        .status(400)
    ...
    let reviewBookData = await reviewModel
...
const updatedBook = async function (req, res) {
  try {
    ...
    if (title) {
      ...
      const findTitle = await bookModel.findOne({
        title: title,
        isDeleted: false,
      });
      if (findTitle) {
    ...
    if (ISBN) {
      ...
      const findIsbn = await bookModel.findOne({
        ISBN: ISBN,
        isDeleted: false,
      });
      if (findIsbn) {
