...
const getBooksData = async function (req, res) {
    ...
    try {
        ...
        if (Object.keys(data).length ==0) {
            getBooks = await bookModel.find({data, isDeleted: false })
            // .select({ _id: 1, title: 1, excerpt: 1, userId: 1, category: 1, reviews: 1, releasedAt: 1, })
            .sort({ title: 1 })
            .populate("userId")
            return res.status(200).send({ status: true, message: 'Books list',length:getBooks.length, data: getBooks })
        }
        ...
        let books = await bookModel.find(bookData)
